...
def filter_by_year(statistics, year, yearid):
    ...
    for row in statistics :
        if dict(row)[(yearid)] == str(year) :
            new_list.append(row)

    return new_list

# print(filter_by_year([{'year2': '2', 'year1': '1', 'year3': '3'}], 1, 'year1'))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    return_list= []
    for row in statistics :
        return_list.append((row[info['playerid']], formula(info, row)))

    return_list.sort(key=lambda pair: pair[1], reverse=True)
    return return_list[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    name_list = []
    csvfile = read_csv_as_nested_dict(info['masterfile'],
    info['playerid'] ,info['separator'], info['quote'])
    for each in top_ids_and_stats :
        for item in csvfile :
            playerid = each[0]
            playerscore = each[1]
            firstname =csvfile[item][info['firstname']]
            lastname = csvfile[item][info['lastname']]
            fullname = firstname +" "+lastname
            if item == playerid :
                name_list.append("{0:.3f} --- {1}".format(playerscore, fullname))
    return name_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    csvfile = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    # print(csvfile)
    filter_year = filter_by_year(csvfile, year, info['yearid'])
    topplayer = top_player_ids(info, filter_year, formula, numplayers)
    lookup = lookup_player_names(info, topplayer)

    return lookup[:numplayers]
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics :
        for field in fields :
 #check if 1 is in the dictionary
            if row[playerid] in return_dict.keys() :
 #if there is, check if 1 has field as a key
                if field in return_dict[row[playerid]].keys() :
                    return_dict[row[playerid]][field] += int(row[field])
 # if 1 doesn't have the field as a key, add
                else : return_dict[row[playerid]].update({field :int(row[field])})
 #if there is no 1,
            else :
                return_dict[row[playerid]] = {playerid :row[playerid], field : int(row[field])}
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    csvfile = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    career_stat = aggregate_by_player_id(csvfile, info['playerid'], info['battingfields'])
    statistics = career_stat.values()
    top_player = top_player_ids(info, statistics, formula, numplayers)
    return_strings = lookup_player_names(info, top_player)
    # print(top_player)
